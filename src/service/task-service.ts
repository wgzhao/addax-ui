...
class TaskService {
  ...
  resetQueue(): Promise<ApiResponse> {
    return Requests.post(`${this.prefix}/queue/actions/reset`, {}) as unknown as Promise<ApiResponse>;
  }
  ...
  executeTask(taskId: number,  timeout = 60000): Promise<ExecuteTaskResponse> {
    return Requests.post(`${this.prefix}/${taskId}/executions`, {}, {timeout}) as unknown as Promise<ExecuteTaskResponse>;
  }
}
