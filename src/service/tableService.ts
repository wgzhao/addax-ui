...
class TableService {
  ...
  // 分页查询采集表
  fetchTableList(page: number, pageSize: number, q: string, runStatus?: string, sortBy?: any): Promise<Page<VEtlWithSource>> {
  ...
  fetchAddaxResult(tableId: number): Promise<EtlStatistic[]> {
    return Requests.get(`${this.prefix}/${tableId}/statistics`) as unknown as Promise<EtlStatistic[]>;
  }
