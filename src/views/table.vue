  ...
  <v-card flat title="采集表配置">
    <template v-slot:text>
      <v-row justify="center" align-content="center">
        ...
        <v-col cols="auto">
          <v-btn variant="tonal" prepend-icon="mdi-plus" @click="openDialog('BatchAdd', 'BatchAdd')">
            新增表
          </v-btn>
        </v-col>
        <v-col cols="auto">
        ...
        <v-col cols="auto">
          <v-btn variant="tonal" prepend-icon="mdi-database" :disabled="selected.length === 0" @click="doEtl(null)">
            批量采集
          </v-btn>
        </v-col>
      </v-row>
      ...
      <v-data-table-server density="compact" :items="table" :headers="headers" :items-per-page="currPageSize"
        ...
        <template v-slot:item.action="{ item }">
          <v-row justify="center" no-gutters>
            <v-btn small density="compact" color="primary" class="mr-1" @click="openDialog('MainTableInfo', item)">
              详情
            </v-btn>
            <v-btn small density="compact" color="secondary" class="mr-1" @click="openDialog('FieldsCompare', item)">
              字段
            </v-btn>
            <v-btn small density="compact" color="info" class="mr-1" @click="openDialog('AddaxJob', item)">
              模板
            </v-btn>
            <v-btn small density="compact" color="success" class="mr-1" @click="openDialog('AddaxResult', item)">
              结果
            </v-btn>
            <v-btn small density="compact" color="info" class="mr-1" @click="openDialog('LogFiles', item)">
              日志
            </v-btn>
...
const doEtl = async (item: any | null) => {
  ...
  try {
    taskService.executeTasksBatch(tids).then(() => {
      // 所有任务提交成功
      notify('采集任务已提交，可在任务中心查看进展', 'success')
    }).catch((err) => {
...
function updateSingleTable(tid: number) {
  tableService.updateSingleTable(tid).then((res) => {
