      ...
      <v-data-table-server density="compact" :items="table" :headers="headers" :items-per-page="currPageSize"
            ...
            <template v-if="item.status === 'U' || item.status === 'E'">
              <v-menu open-on-click :close-on-content-click="false" min-width="220" offset-y>
                <template #activator="{ props }">
                  <v-icon size="16" color="warning" class="ml-1 align-middle cursor-pointer" v-bind="props"
        ...
        <template v-slot:item.action="{ item }">
          <v-row justify="center" no-gutters>
            <v-btn small density="compact" color="primary" class="mr-1" @click="openDialog('TableDetail', item)">
              详情
            </v-btn>
