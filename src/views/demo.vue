<template>
  <v-container>
    ...
    <v-tabs v-model="activeTab">
      <!-- 动态创建每个标签 -->
      <v-tab v-for="tab in tabs" :key="tab.id" class="d-flex align-items-center">
        <!-- 标签标题 -->
        <span>{{ tab.title }}</span>

        <!-- 关闭图标 -->
        <!-- 防止冒泡切换 -->
        <v-btn icon size="x-small" class="ml-2" @click.stop="removeTab(tab.id)">
      ...
      <v-tab-item v-for="tab in tabs" :key="tab.id">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-btn color="secondary" @click="addTab">新建标签页</v-btn>
          </v-card-text>
        </v-card>
      </v-tab-item>
    </v-tabs-items>
