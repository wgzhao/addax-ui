  ...
  <v-card flat title="采集任务管理">
    <v-card-text>
      <v-row align="center" class="mb-2" style="gap: 12px;">
        ...
        <v-btn small color="secondary" class="ma-0" @click="loadSampleData" title="加载示例数据">示例数据</v-btn>
        <v-select v-model="selectedInterval" :items="intervalOptions" item-title="label" item-value="value" dense
          hide-details style="width: 180px" label="自动刷新间隔"></v-select>
  ...
  (cur, prev) => {
    cur.forEach((c, idx) => {
      ...
      if (c.progress !== p.progress) {
        console.debug(`[task.vue] progress changed for ${c.id}: ${p.progress} -> ${c.progress} @ ${new Date().toISOString()}`)
      }
    })
  },
  { deep: true }
