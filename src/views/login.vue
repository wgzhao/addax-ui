    ...
    <v-sheet class="login-sheet pa-16" rounded="xl" elevation="24">
      <v-card class="login-card px-10 py-12" max-width="480">
        ...
        <v-form v-model="form" @submit.prevent="login">
          <v-text-field v-model="auth.username" :readonly="loading" :rules="[required]" class="modern-input mb-8"
            clearable label="Username/Email" prepend-inner-icon="mdi-account-circle" variant="outlined"
            density="comfortable" flat color="cyan" bg-color="rgba(255, 255, 255, 0.1)"
            base-color="white"></v-text-field>

          <v-text-field type="password" v-model="auth.password" :readonly="loading" :rules="[required]"
            class="modern-input mb-10" clearable label="Password" prepend-inner-icon="mdi-lock" variant="outlined"
            density="comfortable" flat color="cyan" bg-color="rgba(255, 255, 255, 0.1)"
            base-color="white"></v-text-field>

          <v-btn :disabled="!form" :loading="loading" block color="primary" size="large" type="submit"
            variant="elevated" class="login-btn">
            <span>Login</span>
...
.tech-particles {
  ...
  width: 100%;
  height: 100%;
  background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600"><circle cx="100" cy="100" r="2" fill="rgba(255,255,255,0.2)"/><circle cx="700" cy="500" r="3" fill="rgba(255,255,255,0.15)"/><circle cx="400" cy="300" r="2" fill="rgba(255,255,255,0.25)"/></svg>') repeat;
