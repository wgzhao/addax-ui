      ...
      <v-card-text>
        <v-data-table :items="sources" :headers="headers" :search="searchValue" density="compact" items-per-page="20">
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn color="secondary" class="btn btn-xs btn-info me-2" @click="doAction(item.id, 'show')">
              详情
            </v-btn>
            <v-btn color="primary" class="btn btn-xs btn-warning me-2" @click="doAction(item.id, 'edit')">
              编辑
            </v-btn>
            <v-btn color="error" class="btn btn-xs btn-danger" @click="openDeleteDialog(item.id, item.name)">
              删除
            </v-btn>
